...
class tax_calculations(models.Model):
    ...
    @api.onchange('total_declared_80c','total_declared_vi_a_deductions','total_declared_hra','total_declared_medical','total_exemption','total_declared_other','total_tax_previous_employer')
    def onchange_total_calculation(self):
        self.grand_total=self.total_declared_80c +self.total_declared_vi_a_deductions +self.total_declared_hra + self.total_declared_medical + self.total_exemption +self.total_declared_other + self.total_tax_previous_employer

    def caclculate_it_declaration(self):
        ...
        print("current_financial_year ==",current_financial_year)
        if active_employee_data:
            for emp_rec in active_employee_data:
                it_declation_id = self.env['hr.payslip'].search([('employee_id', '=', emp_rec.id),('financial_year','=',current_financial_year)])
                print("it_declation_id == ",it_declation_id)
